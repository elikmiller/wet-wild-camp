...
module.exports = async (req, res, next) => {
    try {
        ...
        let ageOnStartDate = moment.utc(camp.startDate).diff(moment.utc(camper.dateOfBirth), "years", false);

        if (camp.type === "adventure" && (ageOnStartDate < 9 || ageOnStartDate > 15)) {
            return next(Boom.badRequest("This camper is not the correct age for this camp."));
        }
        ...
        if (camp.type === "all" && (ageOnStartDate < 8 || ageOnStartDate > 15)) {
            return next(Boom.badRequest("This camper is not the correct age for this camp."));
        }
    ...
    <br/>
    <p>You may review your registrations and payments any time by visiting <a href="${process.env.CORS_URL}">${process.env.CORS_URL}</a></p>
    `
            : `
    <p>Dear ${user.firstName},</p>
    ...
    `;
        ...
        let subjectLine = waitlist ? "Waitlist Confirmation" : "Registration Confirmation";
