...
class AdminRegistrationDetail extends Component {
    ...
    getRegistration = () => {
        appClient.adminGetRegistration(this.props.match.params.registrationId).then((registration) => {
            this.setState({
    ...
    deleteRegistration = () => {
        ...
        appClient.adminDeleteRegistration(this.state.registration._id).then(() => {
            if (location.state && location.state.previousPage) {
                history.push(location.state.previousPage);
            } else history.push("/admin/registrations");
        });
    };
            ...
            <div className="admin-registration-detail">
                <div className="card">
                        ...
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <UncontrolledDropdown>
                                    ...
                                    <DropdownMenu right>
                                        <DropdownItem onClick={this.editOpen}>Edit Registration</DropdownItem>
                    ...
                    <div className="card-body">
                        ...
                        <div className="mb-3">
                            <strong>User: </strong>
                            <Link to={`/admin/users/${registration.user._id}`}>{registration.user.fullName}</Link>
                        </div>

                        <div className="mb-3">
                            <strong>Camper: </strong>
                            <Link to={`/admin/campers/${registration.camper._id}`}>{registration.camper.fullName}</Link>
                        </div>

                        <div className="mb-3">
                            <strong>Camp: </strong>
                            <Link to={`/admin/camps/${registration.camp._id}`}>{registration.camp.fullName}</Link>
                        </div>
                    </div>
                </div>
                <Modal isOpen={this.state.confirmDeleteIsOpen} toggle={this.confirmDeleteClose}>
                    <ModalHeader toggle={this.confirmDeleteClose}>Delete Camper</ModalHeader>
                    <ModalBody>Are you sure you want to delete this registration?</ModalBody>
                    <ModalFooter>
                        <button className="btn btn-outline-secondary mr-3" onClick={this.confirmDeleteClose}>
                            Cancel
                        </button>
                        <button className="btn btn-danger" onClick={this.deleteRegistration}>
                            Delete
                        </button>
                    </ModalFooter>
                </Modal>
